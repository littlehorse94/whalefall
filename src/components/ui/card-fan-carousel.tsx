      ...
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            const image = (
              <div className={`relative overflow-hidden rounded-2xl shadow-xl ${CARD_SIZE_CLASSES}`}>
                <Image
                  src={card.imgUrl}
                  alt={card.alt || `Card ${index}`}
                  fill
                  sizes="(max-width: 480px) 96px, (max-width: 640px) 128px, (max-width: 768px) 160px, (max-width: 1024px) 192px, 192px"
            ...
            if (card.linkUrl) {
              return (
                <a key={index} className="fan-card block cursor-pointer" href={card.linkUrl} target={card.linkUrl.startsWith('http') ? '_blank' : '_self'} rel="noopener noreferrer">
                  {image}
                </a>
              );
            }
            return (
              <div
                ...
                onKeyDown={(e) => {
                  if (onCardClick && (e.key === 'Enter' || e.key === ' ')) onCardClick(card, index);
                }}
              >
                {image}
              </div>
